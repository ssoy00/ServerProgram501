//axios 통신 테스트용 함수
async function getTest(mno) {
    const result = await axios.get(`/replies/list/${mno}`)
...
async function getList({mno,page,size,goLast}){
   const result = await axios.get(`/replies/list/${mno}`,{params : {page,size}})

    // 마지막 댓글 보러가기
    if(goLast) {
        const total = result.data.total
        const lastPage = parseInt(Math.ceil(total/size))
        return getList({mno:mno, page:lastPage, size:size})
    }
